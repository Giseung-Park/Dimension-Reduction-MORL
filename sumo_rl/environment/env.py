...
class SumoEnvironment(gym.Env):
    ...
    def __init__(
        ...
        reward_fn: Union[str, Callable, dict] = "waiting-time-lane", # "waiting-time-road", "waiting-time-road-big"
    ...
    def reward_dimension(self):
        """Return the reward dimension of sumo-rl env.
        """
        if self.reward_fn == "waiting-time-lane":
            return 16
        else:
    ...
    def _get_per_agent_info(self):
        stopped = [self.traffic_signals[ts].get_total_queued() for ts in self.ts_ids] ## OK
        # accumulated_waiting_time = [
        #     sum(self.traffic_signals[ts].get_accumulated_waiting_time_per_lane()) for ts in self.ts_ids
        # ]

        ### Original - standard 2-way-intersection environment
        # per_road_waiting_time = [self.traffic_signals[ts].get_accumulated_waiting_time_per_road() for ts in self.ts_ids]

        # Big-intersection environment
        per_road_waiting_time = [self.traffic_signals[ts].get_accumulated_waiting_time_per_road_big() for ts in self.ts_ids]

        # ## Added - consider power
        # per_road_waiting_time = np.array(per_road_waiting_time) / 100
        # per_road_waiting_time = np.power(per_road_waiting_time, 1.5).tolist() ## traffic penalty
        # ##
        # lane_list = [self.traffic_signals[ts].lanes for ts in self.ts_ids]
        accumulated_waiting_time = [sum(item) for item in per_road_waiting_time]
        min_per_road_waiting_time = [min(item) for item in per_road_waiting_time]
        max_per_road_waiting_time = [max(item) for item in per_road_waiting_time]
        average_speed = [self.traffic_signals[ts].get_average_speed() for ts in self.ts_ids] ## OK
        info = {}
        for i, ts in enumerate(self.ts_ids):
            ...
            info[f"{ts}_min_per_road_waiting_time"] = min_per_road_waiting_time[i] ### New
            info[f"{ts}_average_speed"] = average_speed[i]
        info["total_stopped"] = sum(stopped)
        info["total_average_speed"] = sum(average_speed)
        info["total_accumulated_waiting_time"] = sum(accumulated_waiting_time)
        info["total_min_per_road_waiting_time"] = sum(min_per_road_waiting_time) ### New
        info["total_max_per_road_waiting_time"] = sum(max_per_road_waiting_time) ### New
        if self.single_agent: ### New
            for idx in range(4): ### Per road. intersection case.
                info["total_per_road_waiting_time_" + str(idx+1)] = sum([item[idx] for item in per_road_waiting_time])
        return info
