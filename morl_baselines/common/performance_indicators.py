...
def hypervolume(ref_point: np.ndarray, points: List[npt.ArrayLike]) -> float:
    ...
    """
    return HV(ref_point=ref_point * -1)(np.array(points) * -1) # original
...
def sparsity(front: List[np.ndarray]) -> float:
    ...
    sparsity_value = 0.0
    normalized_sparsity_value = 0.0
    m = len(front[0])
    front = np.array(front)
    for dim in range(m):
        objs_i = np.sort(deepcopy(front.T[dim]))
        max_min_gap_i = np.max(objs_i) - np.min(objs_i)
        for i in range(1, len(objs_i)):
            sparsity_value += np.square(objs_i[i] - objs_i[i - 1])
            if max_min_gap_i > 0:
                normalized_sparsity_value += np.square( (objs_i[i] - objs_i[i - 1]) / max_min_gap_i  )
    sparsity_value /= len(front) - 1
    normalized_sparsity_value /= len(front) - 1

    return sparsity_value, normalized_sparsity_value
...
def expected_utility(front: List[np.ndarray], weights_set: List[np.ndarray], utility: Callable = np.dot) -> float:
    ...
    return np.mean(np.array(maxs), axis=0), np.std(np.array(maxs), axis=0)
