...
def eval_mo(
    ...
    ## For ours (reward dim reduction) + baselines
    vec_return, disc_vec_return = np.zeros(ori_rew_dim), np.zeros(ori_rew_dim)
    gamma = 0.99 # not 1.0
    while not done:
        if render:
            env.render()
    ...
    if w is None:
        ...
        try:
            ...
            scalarized_return, scalarized_discounted_return = 0,0 # dummy
...
def policy_evaluation_mo(agent, env, ori_rew_dim: int, w: np.ndarray, rep: int = 5, seed: int = 0) -> Tuple[float, float, np.ndarray, np.ndarray]:
    ...
    """
    evals = [eval_mo(agent, env, ori_rew_dim, w, seed) for _ in range(rep)]
...
def log_all_multi_policy_metrics(
    ...
    # hv = hypervolume(hv_ref_point, filtered_front) # original

    hv = []
    for ref in hv_ref_point:
        hv.append(hypervolume(ref, filtered_front))

    sp, norm_sp = sparsity(filtered_front)
    eum, eum_std = expected_utility(filtered_front, weights_set=eval_w)
    # eum = expected_utility(filtered_front, weights_set=equally_spaced_weights(reward_dim, n_sample_weights))

    wandb.log(
        {
            # "eval/hypervolume": hv,
            "eval/hypervolume1": hv[0],
            "eval/hypervolume2": hv[1],
            "eval/hypervolume3": hv[2],
            "eval/sparsity": sp,
            "eval/normalized_sparsity": norm_sp,
            "eval/num_filtered_pt": len(filtered_front),
            "eval/eum": eum,
            "eval/eum_std": eum_std,
