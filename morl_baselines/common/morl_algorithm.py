...
class MOPolicy(ABC):
    ...
    def policy_eval(
        ...
        (
            ...
            vec_return,
            discounted_vec_return,
        ) = policy_evaluation_mo(self, eval_env, w=weights, rep=num_episodes, seed=self.seed, ori_rew_dim=ori_rew_dim)
...
class MOAgent(ABC):
    ...
    def __init__(self, env: Optional[gym.Env], device: Union[th.device, str] = "auto", seed: Optional[int] = None,
                 reward_dim: int = 4) -> None:
    ...
    def extract_env_info(self, env: Optional[gym.Env]) -> None:
        ...
        if env is not None:
            ...
            #     self.reward_dim = self.env.reward_dimension
