...
from morl_baselines.multi_policy.envelope.compressors import AE_PositiveEnc, AE_standard, AE_PositiveEnc_Only, NPCA
from morl_baselines.multi_policy.envelope.weight_matrix import WeightMat
...
class Envelope(MOPolicy, MOAgent):
    ...
    def __init__(
        ...
        r_learn_interval: int = 1,
        evaluation_form: str = 'random',  ## 0.0003
        reduce_baselines: str = 'ours',
        relax_positive: bool = False,
        n_coef: float = 1.0,
        drp: float = 0.0,
        dehidnum: int = 32,
    ):
        ...
        self.r_learn_interval = r_learn_interval
        self.evaluation_form = evaluation_form
        self.reduce_baselines = reduce_baselines
        self.relax_positive = relax_positive ## for ours only
        self.drp = drp ## for ours only
        self.n_coef = n_coef ## for NPCA only

        self.dehidnum = dehidnum
        ...
        if self.reward_reduce:
            if self.reduce_baselines == 'ours':
                self.reward_compressor = AE_PositiveEnc(input_size=self.ori_rew_dim, hidden_size=self.reward_dim,
                                                        relax_positive=self.relax_positive, drp=self.drp, dehidnum=self.dehidnum) # change neural network size
            elif self.reduce_baselines == 'ours_nodec' or self.reduce_baselines == 'random':
                self.reward_compressor = AE_PositiveEnc_Only(input_size=self.ori_rew_dim, hidden_size=self.reward_dim, relax_positive=self.relax_positive) # change neural network size
            elif self.reduce_baselines == 'ae':
                self.reward_compressor = AE_standard(input_size=self.ori_rew_dim, hidden_size=self.reward_dim, dehidnum=self.dehidnum)
            elif self.reduce_baselines == 'pca':
                self.reward_compressor = WeightMat(reduced_rew_dim=self.reward_dim, ori_rew_dim=self.ori_rew_dim, device=self.device).to(self.device)
            elif self.reduce_baselines == 'npca':
                self.reward_compressor = NPCA(input_size=self.ori_rew_dim, hidden_size=self.reward_dim)
            else:
                raise NotImplementedError
            # self.w_compressor = Compressor_w(input_size=self.ori_rew_dim, hidden_size=self.reward_dim)

            self.r_optim = optim.Adam(self.reward_compressor.parameters(), lr=self.r_learning_rate, weight_decay=1e-5)
            # self.w_optim = optim.Adam(self.w_compressor.parameters(), lr=self.w_learning_rate)
            self.criterion = nn.MSELoss()
        self.reward_corr_mat = np.zeros((self.ori_rew_dim, self.ori_rew_dim), dtype='float32')  # dtype='float32'
        self.reward_mean = np.zeros(self.ori_rew_dim, dtype='float32')
        self.reward_corr_mat_no_mean = np.zeros((self.ori_rew_dim, self.ori_rew_dim), dtype='float32')  # dtype='float32'
    ...
    def update(self):
        ...
        for g in range(self.gradient_updates):
            ...
            if self.reward_reduce:
                ...
                with th.no_grad():
                    if self.reduce_baselines == 'pca' or self.reduce_baselines == 'npca':
                        b_rewards = self.reward_compressor.forward(b_rewards-th.tensor(self.reward_mean).to(b_rewards.dtype))  # from centered data to mean 0 data
                    elif self.reduce_baselines == 'ours' or self.reduce_baselines == 'ae' or self.reduce_baselines == 'ours_nodec' or self.reduce_baselines == 'random':
    ...
    def train(
        ...
        ref_point: Optional[List[np.ndarray]] = None, #@
        ...
        if self.log:
            ...
            self.register_additional_config({"ref_point": ref_point, "known_front": known_pareto_front})
        ...
        num_episodes = 0
        eval_pairwise_differences = []
        ### Evaluation is conducted for original weight. Fixed.
        if self.evaluation_form == 'equal':
            ...
            # eval_weights = equally_spaced_weights(self.ori_rew_dim, n=num_eval_weights_for_front) # list of N weights
            # eval_weights = equally_spaced_weights(self.reward_dim, n=num_eval_weights_for_front)  # list of N weights
            # eval_w_normalize = equally_spaced_weights(self.ori_rew_dim, n=num_eval_weights_for_front)  # list of N weights
            if self.reward_reduce: # m=3
                if self.reward_dim == 2:
                    eval_weights = np.load('eval_weights/equal_weight_rd=2_q=14_15.npz')
                    eval_weights = [row for row in eval_weights['arr_0']]
                elif self.reward_dim == 3:
                    eval_weights = np.load('eval_weights/equal_weight_rd=3_q=4_15.npz')
                    eval_weights = [row for row in eval_weights['arr_0']]
                ...
                # eval_weights = equally_spaced_weights(self.ori_rew_dim, n=num_eval_weights_for_front)
                eval_weights = np.load('eval_weights/equal_weight_rd=5_q=2_15.npz')
                eval_weights = [row for row in eval_weights['arr_0']]

            # print("eval_weights", eval_weights, eval_weights.shape)
            # print()
            eval_w_normalize = np.load('eval_weights/equal_weight_rd=5_q=5_126.npz')
            eval_w_normalize = [row / np.linalg.norm(row) for row in eval_w_normalize['arr_0']]
            # eval_w_normalize = np.load('eval_weights/equal_weight_rd=16_q=2_136.npz')
            # eval_w_normalize = eval_w_normalize['arr_0']
            # print("eval_w_normalize", eval_w_normalize, eval_w_normalize.shape)
            # print()

            # eval_w_normalize = [weight/np.linalg.norm(weight) for weight in eval_w_normalize]

            # from itertools import combinations
            # pairwise_differences = []
            # # Generate all combinations of row indices (i, j) where i < j
            # combs = list(combinations(range(len(eval_weights)), 2))
            # # Compute the pairwise differences
            # for (i, j) in combs:
            #     diff = eval_weights[i] - eval_weights[j]
            #     pairwise_differences.append(diff)
            # # Convert the list of pairwise differences to a numpy array
            # eval_pairwise_differences = np.array(pairwise_differences) # (190,4)

        elif self.evaluation_form == 'random':
            ## Candidate 2. Dirichlet distribution. Cons: randomness
            # eval_weights = random_weights(self.ori_rew_dim, num_eval_weights_for_front, dist="dirichlet", rng=self.np_random) # (num_eval_weights_for_front, self.ori_rew_dim)
            eval_weights = random_weights(self.reward_dim, num_eval_weights_for_front, dist="dirichlet", rng=self.np_random) # (num_eval_weights_for_front, self.ori_rew_dim)
        else:
            raise NotImplementedError
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            vec_reward = vec_reward.astype('float32')
            self.global_step += 1

            self.replay_buffer.add(obs, action, vec_reward, next_obs, terminated) ## original high-dim vector reward is saved.

            self.count += 1
            self.reward_corr_mat = ((self.count - 1) * self.reward_corr_mat
                                    + (1 - 1 / self.count) * np.outer(vec_reward - self.reward_mean,
                                                                      vec_reward - self.reward_mean)) / self.count
            self.reward_mean = ((self.count - 1) * self.reward_mean + vec_reward) / self.count

            self.reward_corr_mat_no_mean = ((self.count - 1) * self.reward_corr_mat_no_mean + np.outer(vec_reward, vec_reward)) / self.count
            ...
            ### 2. Dimension Reduction Function Update
            if (self.global_step >= self.batch_size) and (self.global_step % self.r_learn_interval == 0):
                ## 1. r_AE update
                if not self.reward_reduce:
                    wandb.log(
                        {
                            "losses/r_loss": 0, #default
                        },
                    )
                    # pass
                elif self.reduce_baselines == 'ours': # self.reward_reduce:
                    (
                        _,
                        _,
                        b_rewards,  # [32, self.ori_rew_dim]
                    ...
                    #         "losses/cos0": self.cos(self.reward_compressor.encoder[0].weight[0], self.reward_compressor.encoder[0].weight[1]).data,
                    #         "losses/cos1": self.cos(self.reward_compressor.encoder[0].weight[0], self.reward_compressor.encoder[0].weight[2]).data,
                    #         "losses/cos2": self.cos(self.reward_compressor.encoder[0].weight[0], self.reward_compressor.encoder[0].weight[3]).data,
                    #         "losses/cos3": self.cos(self.reward_compressor.encoder[0].weight[1], self.reward_compressor.encoder[0].weight[2]).data,
                    #         "losses/cos4": self.cos(self.reward_compressor.encoder[0].weight[1], self.reward_compressor.encoder[0].weight[3]).data,
                    #         "losses/cos5": self.cos(self.reward_compressor.encoder[0].weight[2], self.reward_compressor.encoder[0].weight[3]).data,
                    #     }, # rd=4 case
                    # )
                    wandb.log(
                        {
                ...
                ###
                    ...
                    pca_loss = th.sum((self.reward_compressor.encoder[0].weight @ th.tensor(self.reward_corr_mat).to(self.reward_compressor.encoder[0].weight.dtype)) * self.reward_compressor.encoder[0].weight)
                    ortho_loss = th.sum(th.square(self.reward_compressor.encoder[0].weight @ self.reward_compressor.encoder[0].weight.T - th.eye(self.reward_dim)))

                    r_loss = -(pca_loss - self.n_coef* ortho_loss) # maximize pca_loss while minimize ortho_loss
                ...
                ###
                ...
                elif self.reduce_baselines == 'ours_nodec':
                    weights1 = random_weights(self.reward_dim, 100, dist="dirichlet", rng=self.np_random) # [n, self.reward_dim] = [100,4]
                    weights2 = random_weights(self.reward_dim, 100, dist="dirichlet", rng=self.np_random) # [n, self.reward_dim] = [100,4]
                    weight = th.tensor(weights1 - weights2, dtype=th.float32)

                    # [self.reward_dim, self.reward_dim]
                    center_matrix = self.reward_compressor.encoder[0].weight @ th.tensor(self.reward_corr_mat_no_mean) @ self.reward_compressor.encoder[0].weight.T
                    loss_vec = th.sum((weight @ center_matrix) * weight, dim=-1) # [100]
                    r_loss = -th.mean(th.sqrt(loss_vec)) # we want to maximize th.mean(loss_vec)
                    ...
                    wandb.log(
                        {
                            "losses/disperse_loss": -r_loss.item(),
                        },
                    )
                    # print("A", self.reward_compressor.encoder[0].weight)
                    # print("loss",  -r_loss.item())
                    # print()

                elif self.reduce_baselines == 'pca':
                    eigenvalues, eigenvectors = np.linalg.eigh(self.reward_corr_mat)  # eigenvectors (self.ori_rew_dim, self.ori_rew_dim)
                    sorted_indices = np.argsort(eigenvalues)[::-1]
                    # Select the top-k eigenvalues and eigenvectors
                    self.reward_compressor.learnable_matrix = nn.Parameter(
                        th.tensor(eigenvectors[:, sorted_indices[:self.reward_dim]].T, dtype=th.float32).clone().to(self.device))
                    wandb.log(
                        {
                            "losses/r_loss": 0,  # default
                        },
                    )
                elif self.reduce_baselines == 'random': # do nothing
                    wandb.log(
                        {
                            "losses/r_loss": 0,  # default
                        },
            ...
            #     _, S, _ = np.linalg.svd(self.reward_corr_mat)
            #     print("Final check for singular values of reward correlation", S/S[0])
            #     if self.reward_reduce and self.reduce_baselines == 'ours':
            #         _, S, _ = np.linalg.svd(self.reward_compressor.encoder[0].weight.data.numpy())
            #         print("Final check for singular values of A", S/S[0])
            #     print()
            ...
            if eval_env is not None and self.log and self.global_step % eval_freq == 0:
                ...
                print()
                current_front = [
                    self.policy_eval(eval_env, weights=ew, num_episodes=num_eval_episodes_for_front, log=self.log, ori_rew_dim=self.ori_rew_dim)[3]
                ...
                # eval_r_loss = 0
                # if self.evaluation_form == 'equal':
                #     A = self.reward_compressor.encoder[0].weight.data.numpy()  # (4,16)
                #     eval_center_matrix = A @ self.reward_corr_mat_no_mean @ A.T  # (4,4)
                #     loss_vec = np.sum((eval_pairwise_differences @ eval_center_matrix) * eval_pairwise_differences,
                #                       axis=-1)  # [190]
                #     eval_r_loss = np.mean(np.sqrt(loss_vec))  # we want to maximize th.mean(loss_vec)

                pareto_front = log_all_multi_policy_metrics(
                    current_front=current_front,
                    hv_ref_point=ref_point,
                    reward_dim=self.ori_rew_dim,
                    global_step=self.global_step,
                    ref_front=known_pareto_front,
                    eval_w=eval_w_normalize, # To calculate EUM
                )

                ## Check rank and filtered pareto front
                _, S1, _ = np.linalg.svd(self.reward_corr_mat)
                print("Check for singular values of reward correlation", S1 / S1[0])
                _, S2, _ = np.linalg.svd(self.reward_corr_mat_no_mean)
                print("Check for singular values of reward correlation with no mean", S2 / S2[0])
                if self.reward_reduce and (self.reduce_baselines == 'ours' or self.reduce_baselines == 'npca' or self.reduce_baselines == 'random'):
                    _, S3, _ = np.linalg.svd(self.reward_compressor.encoder[0].weight.data.numpy())
                    filename = (str(self.dehidnum) + "_" +
                                self.reduce_baselines + str(self.reward_dim) + str(self.relax_positive) + str(self.drp)
                                + str(self.r_learning_rate) + str(self.r_learn_interval) + '_step=' + str(self.global_step) + '_seed=' + str(self.seed))
                    np.savez('archive/' + filename + '.npz',
                             self.reward_corr_mat, S1 / S1[0], self.reward_corr_mat_no_mean, S2 / S2[0],
                             self.reward_compressor.encoder[0].weight.data.numpy(), S3 / S3[0])
                else:
                    filename = (self.reduce_baselines + str(self.reward_dim) + '_hodec=' + str(self.homotopy_decay_steps)
                                + '_step=' + str(self.global_step) + '_seed=' + str(self.seed))
                    np.savez('archive/' + filename + '.npz',
                             self.reward_corr_mat, S1 / S1[0], self.reward_corr_mat_no_mean, S2 / S2[0])

                np.savez('archive_return/LunarLander_Rev3/' + filename + '.npz', np.array(pareto_front))
            ...
            if terminated or truncated:
                ...
                if weight is None:
                    ...
                    # w = random_weights(self.ori_rew_dim, 1, dist="dirichlet", rng=self.np_random)
        ...
        self.save(save_replay_buffer=False, save_dir='archive_model', filename=filename)