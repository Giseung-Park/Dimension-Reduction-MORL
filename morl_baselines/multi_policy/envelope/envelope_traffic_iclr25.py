...
class Envelope(MOPolicy, MOAgent):
        ...
        if self.reward_reduce:
            if self.reduce_baselines == 'ours':
                self.reward_compressor = AE_PositiveEnc(input_size=self.ori_rew_dim, hidden_size=self.reward_dim,
                                                        relax_positive=self.relax_positive, drp=self.drp) # change neural network size
                ...
                self.reward_compressor = AE_standard(input_size=self.ori_rew_dim, hidden_size=self.reward_dim)
            ...
            if self.reward_reduce:
                ...
                with th.no_grad():
                    if self.reduce_baselines == 'pca' or self.reduce_baselines == 'npca':
                        b_rewards = self.reward_compressor.forward(b_rewards-th.tensor(self.reward_mean))  # from centered data to mean 0 data
    ...
    def train(
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            if (self.global_step >= self.batch_size) and (self.global_step % self.r_learn_interval == 0):
                ...
                ###
                    ...
                    pca_loss = th.sum((self.reward_compressor.encoder[0].weight @ th.tensor(self.reward_corr_mat)) * self.reward_compressor.encoder[0].weight)
        ...
        print("Final check for singular values of reward correlation", S1/S1[0])
        _, S2, _ = np.linalg.svd(self.reward_corr_mat_no_mean)
        print("Final check for singular values of reward correlation with no mean", S2/S2[0])
        if self.reward_reduce and (self.reduce_baselines == 'ours' or self.reduce_baselines == 'npca' or self.reduce_baselines == 'random'):
            ...
            filename = (self.reduce_baselines + str(self.reward_dim)  + str(self.relax_positive) + str(self.drp)
                        + str(self.r_learning_rate) + str(self.r_learn_interval) +  '_' + str(self.seed))
            np.savez('archive/' + filename + '.npz',
                     self.reward_corr_mat, S1 / S1[0], self.reward_corr_mat_no_mean, S2 / S2[0], self.reward_compressor.encoder[0].weight.data.numpy(), S3/S3[0])
        else:
            filename = self.reduce_baselines + str(self.reward_dim)  + '_' + str(self.seed)
            np.savez('archive/' + filename + '.npz',
                     self.reward_corr_mat, S1 / S1[0], self.reward_corr_mat_no_mean, S2 / S2[0])
        self.save(save_replay_buffer=False, save_dir='model_archive', filename=filename)