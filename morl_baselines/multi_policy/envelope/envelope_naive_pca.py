...
class Envelope(MOPolicy, MOAgent):
    ...
    def __init__(
        ...
        update_matrix_freq_factor: int =1, ##
    ):
        ...
        """
        MOAgent.__init__(self, env, device=device, seed=seed, reward_dim=reduced_reward_dim)
        ...
        if self.reward_dim < self.ori_rew_dim:
            self.weight_matrix = WeightMat(reduced_rew_dim=self.reward_dim, ori_rew_dim=self.ori_rew_dim, device=self.device).to(self.device)

            self.update_matrix_freq_factor = update_matrix_freq_factor
            ## We require this value no less than reduced_reward_dim
            # assert self.update_matrix_freq_factor >= self.reward_dim

            self.sample_corr_mat = np.zeros((self.ori_rew_dim, self.ori_rew_dim))
            self.u_n_eig_vec = np.zeros((self.ori_rew_dim, self.reward_dim))
            self.d_n_eig_val = np.zeros((self.reward_dim, self.reward_dim))
    ...
    def update(self):
        ...
        for g in range(self.gradient_updates):
            ...
            if self.reward_dim < self.ori_rew_dim:
                b_rewards = self.weight_matrix.forward(b_rewards) # no grad due to with torch.no_grad():
    ...
    def train(
        ...
        # ## for test
        # eval_weights = np.array([[0.25,0.25,0.25,0.25], [0,0,0,1]])

        eval_weights = np.repeat(eval_weights, 4, axis=1)/4 # number of lanes per road=4 # [35,16]

        obs, _ = self.env.reset()

        ### Training is conducted using reduced weight. For naive method, sample high-dim weight.
        w = weight if weight is not None else random_weights(self.reward_dim, 1, dist="gaussian", rng=self.np_random)
        tensor_w = th.tensor(w).float().to(self.device)

        self.dim_reduction = dim_reduction
        self.weight_dimension_reduction = weight_dimension_reduction

        if self.dim_reduction:
            ### Add a new metric
            # self.undiscounted_return_list = [0.0]
            # self.discounted_return_list = [0.0]
            self.reward_list = []
            self.count = 0
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            # if self.dim_reduction: ## return PCA version
            #     ### calculate discounted cumulative return. High_dimension space.
            #     self.undiscounted_return_list[-1] += vec_reward
            #     self.discounted_return_list[-1] += (self.gamma ** self.count) * vec_reward
            #     assert len(self.undiscounted_return_list) == len(self.discounted_return_list)
            #     self.count += 1

            # ### Incremental PCA and evaluation is conducted in the original space.
            # if self.log and self.global_step % eval_freq == 0: # 30 episodes
            #     log_all_multi_policy_metrics(
            #         current_front=self.discounted_return_list, ## cumulative
            #         hv_ref_point=ref_point,
            #         reward_dim=self.reward_dim, ## original reward dimension
            #         global_step=self.global_step,
            #     )
            ...
            if self.dim_reduction:
                if np.linalg.norm(vec_reward) > 0: ## only consider non_zero vectors since zero vectors does not have meaning.
                    ...
                    if len(self.reward_list) == self.update_matrix_freq_factor:
                        ## update weight using Incremental PCA

                        np_disc_ret = np.array(self.reward_list).T  # [self.ori_rew_dim(=d=16), self.update_matrix_freq_factor(=r)]
                        assert np_disc_ret.shape[1] == self.update_matrix_freq_factor

                        if np.linalg.matrix_rank(self.sample_corr_mat) < self.reward_dim:
                            # [self.ori_rew_dim, self.ori_rew_dim(=16)]. Sample Correlation matrix update equation.
                            self.sample_corr_mat = ((
                                                                self.count - self.update_matrix_freq_factor) * self.sample_corr_mat
                                                    + np.matmul(np_disc_ret, np_disc_ret.T)) / self.count

                            if np.linalg.matrix_rank(self.sample_corr_mat) >= self.reward_dim:
                                ## real symmetric square matrix has always real eigenvalues, and so is eigenvectors.
                                eigenvalues, eigenvectors = np.linalg.eigh(
                                    self.sample_corr_mat)  # we know that self.sample_corr_mat is real symmetric.

                                sorted_indices = np.argsort(eigenvalues)[::-1]  # default ascending, add [::-1] to descend
                                # Select the top-k eigenvalues and eigenvectors
                                self.d_n_eig_val = np.diag(eigenvalues[sorted_indices[:self.reward_dim]])
                                self.u_n_eig_vec = eigenvectors[:, sorted_indices[
                                                                   :self.reward_dim]]  # (d, q=self.reward_dim) I checked that U^T U = I_q.

                        else:  # np.linalg.matrix_rank(self.sample_corr_mat) >= self.reward_dim
                            c_vectors = np.matmul(self.u_n_eig_vec.T, np_disc_ret)  # c_n = U_n^T x_{n+r} [q x r] @@@

                            ### sequentially update.
                            perp_vectors = np_disc_ret - np.matmul(self.u_n_eig_vec, c_vectors)  # [self.ori_rew_dim x r]

                            nonzero_col_index = np.nonzero(np.linalg.norm(perp_vectors, axis=0))[0]
                            nonzero_col_num = len(nonzero_col_index)
                            if nonzero_col_num > 0: # There is a vector which cannot be perfectly represented by U_n. Otherwise, we skip.

                                if nonzero_col_num < self.update_matrix_freq_factor:
                                    np_disc_ret = np_disc_ret[:,nonzero_col_index]
                                    c_vectors = c_vectors[:, nonzero_col_index]
                                    perp_vectors = perp_vectors[:,nonzero_col_index]

                                if nonzero_col_num > 1: ##  Gram-Schmidt Process
                                    for idx in range(nonzero_col_num - 1):
                                        for j in range(idx + 1):
                                            perp_vectors[:, idx + 1] -= self.vector_projection(projdir=perp_vectors[:, j],
                                                                                               origin=np_disc_ret[:, idx + 1])
                                # now, perp_vectors has a size of  [self.ori_rew_dim x m], and 1 <= m <= nonzero_col_num <= self.update_matrix_freq_factor
                                # Important notice is that m < nonzero_col_num is possible.
                                    nonzero_col_index_perp = np.nonzero(np.linalg.norm(perp_vectors, axis=0))[0]
                                    nonzero_col_num_perp = len(nonzero_col_index_perp) # This is m
                                    assert nonzero_col_num_perp >= 1

                                    if nonzero_col_num_perp < nonzero_col_num:
                                        np_disc_ret = np_disc_ret[:,nonzero_col_index_perp] # [d x m]
                                        c_vectors = c_vectors[:,nonzero_col_index_perp] # [q x m]
                                        perp_vectors = perp_vectors[:,nonzero_col_index_perp]  # [d x m]
                                else:
                                    nonzero_col_num_perp = 1 # (==1)

                                norm_perp_vec = perp_vectors / np.linalg.norm(perp_vectors, axis=0)  # broadcasting; # [self.ori_rew_dim x m]. used for eigenvectors

                                # pdb.set_trace()
                                ## Construct a block matrix
                                # [q x q] = [self.reward_dim x self.reward_dim]
                                upper_left = ( self.count - self.update_matrix_freq_factor) * self.d_n_eig_val + np.matmul(
                                    c_vectors, c_vectors.T)

                                proj_coefficient = np.tril(np.matmul(np_disc_ret.T, norm_perp_vec))  # [m x m]
                                upper_right = np.matmul(c_vectors, proj_coefficient)  # [q x m]
                                lower_right = np.matmul(proj_coefficient.T, proj_coefficient)

                                ## self.count revision
                                self.count = self.count - self.update_matrix_freq_factor + nonzero_col_num_perp # [n+m=(n+r)-r+m], not [n+r]

                                block_q = np.block([[upper_left, upper_right],
                                                    [upper_right.T, lower_right]]) / self.count  # (q+m, q+m). divide: n+m

                                eigenvalues, eigenvectors = np.linalg.eigh(block_q) # eigenvectors (q+r, q+r)
                                intermediate_eig_vecs = np.matmul(np.hstack((self.u_n_eig_vec, norm_perp_vec)),
                                                                  eigenvectors)  # [d x (q+r)]*[(q+r)*(q+r)] = [d x (q+r)]
                                # we checked that col vectors in intermediate_eig_vecs are orthonormal.

                                sorted_indices = np.argsort(eigenvalues)[::-1]
                                # Select the top-k eigenvalues and eigenvectors
                                self.d_n_eig_val = np.diag(eigenvalues[sorted_indices[:self.reward_dim]])
                                self.u_n_eig_vec = intermediate_eig_vecs[:, sorted_indices[:self.reward_dim]]

                                # # Convert NumPy arrays to PyTorch tensors and place them on the desired device
                                # self.d_n_eig_val = th.tensor(self.d_n_eig_val, dtype=th.float32, device=self.device)
                                # self.u_n_eig_vec = th.tensor(self.u_n_eig_vec.T, dtype=th.float32, device=self.device)
                                #
                                # Update the learnable matrix
                                # self.weight_matrix.learnable_matrix = nn.Parameter(self.u_n_eig_vec.clone())
                                # self.weight_matrix.learnable_matrix = nn.Parameter(th.tensor(self.u_n_eig_vec.T).clone().to(self.device))
                                self.weight_matrix.learnable_matrix = nn.Parameter(
                                    th.tensor(self.u_n_eig_vec.T, dtype=th.float32).clone().to(self.device))

                        #### Re-initialize "return buffers"
                        # self.undiscounted_return_list = []
                        # self.discounted_return_list = []
                        self.reward_list = []

                if terminated or truncated:
                    self.reward_list = []
                    self.count = 0
                # self.undiscounted_return_list.append(0.0)
                # self.discounted_return_list.append(0.0)
            ...
            if eval_env is not None and self.log and self.global_step % eval_freq == 0:
                ...
                if self.dim_reduction:
                    ...
                    current_front = [
                        self.policy_eval(eval_env, weights=rew, num_episodes=num_eval_episodes_for_front, log=self.log)[3]
                        for rew in reduced_eval_weights
                    ]
                ...
                # pdb.set_trace()
                eum = np.mean(np.sum(eval_weights * np.array(current_front), axis=1))
            ...
            if terminated or truncated:
                ...
                # if self.dim_reduction: ## return PCA version
                #     if self.num_episodes % self.update_matrix_freq_factor == 0:
                #         ## update weight using Incremental PCA
                #
                #         np_disc_ret = np.array(self.discounted_return_list).T # [self.ori_rew_dim(=d=16), self.update_matrix_freq_factor(=r)]
                #         assert np_disc_ret.shape[1] == self.update_matrix_freq_factor
                #
                #         if np.linalg.matrix_rank(self.sample_corr_mat) < self.reward_dim:
                #             # [self.ori_rew_dim, self.ori_rew_dim(=16)]. Sample Correlation matrix update equation.
                #             self.sample_corr_mat = ( (self.num_episodes - self.update_matrix_freq_factor) * self.sample_corr_mat
                #                                     + np.matmul(np_disc_ret, np_disc_ret.T) )/self.num_episodes
                #
                #             if np.linalg.matrix_rank(self.sample_corr_mat) >= self.reward_dim:
                #                 ## real symmetric square matrix has always real eigenvalues, and so is eigenvectors.
                #                 eigenvalues, eigenvectors = np.linalg.eigh(self.sample_corr_mat) # we know that self.sample_corr_mat is real symmetric.
                #
                #                 sorted_indices = np.argsort(eigenvalues)[::-1] # default ascending, add [::-1] to descend
                #                 # Select the top-k eigenvalues and eigenvectors
                #                 self.d_n_eig_val = np.diag(eigenvalues[sorted_indices[:self.reward_dim]])
                #                 self.u_n_eig_vec = eigenvectors[:, sorted_indices[:self.reward_dim]] # (d, q=self.reward_dim) I checked that U^T U = I_q.
                #
                #         else: # np.linalg.matrix_rank(self.sample_corr_mat) >= self.reward_dim
                #             c_vectors = np.matmul(self.u_n_eig_vec.T, np_disc_ret) # c_n = U_n^T x_{n+r} [q x r] @@@
                #
                ...
                #             perp_vectors = np_disc_ret - np.matmul(self.u_n_eig_vec, c_vectors) # [self.ori_rew_dim x r]
                #
                #             if self.update_matrix_freq_factor > 1:
                #                 for idx in range(self.update_matrix_freq_factor-1):
                #                     for j in range(idx+1):
                #                         perp_vectors[:, idx+1] -= self.vector_projection(projdir=perp_vectors[:, j], origin=np_disc_ret[:, idx+1])
                #
                #             norm_perp = np.linalg.norm(perp_vectors, axis=0) # shape (r,) @@@
                #             norm_perp_vec = perp_vectors / norm_perp # broadcasting; # [self.ori_rew_dim x r]. used for eigenvectors
                #
                #             ## Construct a block matrix
                #             # [q x q] = [self.reward_dim x self.reward_dim]
                #             upper_left = (self.num_episodes - self.update_matrix_freq_factor) * self.d_n_eig_val + np.matmul(c_vectors, c_vectors.T)
                #
                #             proj_coefficient = np.tril(np.matmul(np_disc_ret.T, norm_perp_vec)) # [r x r]
                #             upper_right = np.matmul(c_vectors, proj_coefficient) # [q x r]
                #             lower_right = np.matmul(proj_coefficient.T, proj_coefficient)
                #
                #             block_q = np.block([[upper_left, upper_right],
                #                                 [upper_right.T, lower_right]]) / self.num_episodes # (q+r, q+r)
                #
                #             eigenvalues, eigenvectors = np.linalg.eigh(block_q)
                #             intermediate_eig_vecs = np.matmul(np.hstack((self.u_n_eig_vec, norm_perp_vec)), eigenvectors) # [d x (q+r)]*[(q+r)*(q+r)] = [d x (q+r)]
                #             # we checked that col vectors in intermediate_eig_vecs are orthonormal.
                #
                #             sorted_indices = np.argsort(eigenvalues)[::-1]
                #             # Select the top-k eigenvalues and eigenvectors
