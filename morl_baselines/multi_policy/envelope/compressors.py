...
#
#         self.decoder = nn.Sequential(
#             nn.Linear(hidden_size, dehidnum),
#             nn.ReLU(),
#             nn.Dropout(drp),
#             nn.Linear(dehidnum, dehidnum),
#             nn.ReLU(),
#             nn.Dropout(drp),
#             nn.Linear(dehidnum, input_size) # default: bias=True
#         )
#
#     def forward(self, x):
#         x = self.encoder(x)
...
class AE_PositiveEnc(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, relax_positive: bool, drp: float, dehidnum: int):
        super(AE_PositiveEnc, self).__init__()
...
class AE_standard(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, dehidnum: int):
        super(AE_standard, self).__init__()

        # Encoder
        self.encoder = nn.Sequential(
            nn.Linear(input_size, dehidnum),
            nn.ReLU(),
            nn.Linear(dehidnum, dehidnum),
            nn.ReLU(),
            nn.Linear(dehidnum, hidden_size)
        )
